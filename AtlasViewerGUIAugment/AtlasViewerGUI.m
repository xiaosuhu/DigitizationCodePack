...
if (isempty(find(strcmpi(atlasViewer.refpts.labels, 'lpa'))) & isempty(find(strcmpi(atlasViewer.refpts.labels, 'rpa'))))    
    uicontrol('parent',hf, 'style','text', 'string',sprintf(['TIP: If original volume was processed by ', ...
        'Freesurfer, use mri_info to get the 3 letter orientation code, to determine left and right sides.']),...
        'fontsize', 10, 'tag','texttip', 'units','normalized', 'position',[.82,.82,.15,.15]);
end
...
% and fixed (mostly fixed) in R2016a (9.0)
